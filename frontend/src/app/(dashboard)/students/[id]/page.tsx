...
import {
  ...
  CheckCircle2, DollarSign, Receipt, History, ArrowRightLeft,
...
export default function StudentDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // 9. Transfer group mutation
  const transferMutation = useMutation({
    mutationFn: async () => {
      const resp = await api.post(`/api/students/${studentId}/transfer-group`, {
        from_group_id: transferData.from_group_id,
    ...
    onError: (err: { response?: { data?: { message?: string } } }) => {
      setTransferError(err.response?.data?.message || "Guruhni almashtirishda xatolik yuz berdi");
    },
  });
  ...
  const handleOpenTransferModal = (fromGroupId: number, fromGroupName: string) => {
    setTransferData({
  ...
  const handleExportPlans = () => {
    const headers = ['Guruh', 'Kurs', 'Oy', 'Rejadagi summa', "To'langan summa", 'Qoldiq qarz', 'Holati'];
    const rows = paymentPlans.map((p) => [
      p.group?.name || `Guruh #${p.group_id}`,
      p.group?.course?.name || '-',
      p.month,
      p.amount,
      p.paid_amount,
      p.remaining_amount,
      p.status === 'paid' ? "To'langan" : p.status === 'partial' ? "Qisman to'langan" : p.status === 'overdue' ? "Muddati o'tgan" : "Kutilmoqda",
    ]);
    exportToCSV(`${student.name}_to'lov_rejalari`, headers, rows);
  };

  const handleExportHistory = () => {
    const headers = ['Chek №', 'Sana', 'Guruh', 'Oy', 'Summa', "To'lov usuli", 'Qabul qiluvchi', 'Izoh'];
    const rows = paymentHistory.map((item) => [
      ...
      item.receivedBy?.name || 'Admin',
      item.note || '',
    ]);
    exportToCSV(`${student.name}_to'lov_kvitansiyalari`, headers, rows);
  };
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
              ...
              >
                ...
                <div className="flex flex-col items-end gap-1.5 shrink-0">
                  ...
                  {canManage && g.status === 'active' && (
                    <button
                      type="button"
                      onClick={() => handleOpenTransferModal(g.group_id, g.group_name)}
                      className="inline-flex items-center gap-1 text-[11px] font-semibold text-blue-600 dark:text-blue-400 hover:underline pt-0.5 cursor-pointer"
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 space-y-6">
        ...
        {/* Sub-tab Switcher: Plans vs History + Excel Export */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-gray-100 dark:border-gray-750 pb-2">
          <div className="flex items-center gap-2">
            <button
              type="button"
          ...
          <button
            type="button"
            onClick={paymentSubTab === 'plans' ? handleExportPlans : handleExportHistory}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-750 text-xs font-semibold transition cursor-pointer self-start sm:self-auto"
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-fade-in border dark:border-gray-700">
            ...
            >
              ...
              <div className="p-3.5 rounded-xl bg-gray-50 dark:bg-gray-750 border border-gray-200 dark:border-gray-700 text-xs">
                <span className="text-gray-400 block mb-0.5">Joriy guruh:</span>
                <span className="font-bold text-gray-800 dark:text-gray-200 text-sm">
                  {transferData.from_group_name || `Guruh #${transferData.from_group_id}`}
                </span>
              </div>

              <div>
                <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">
                  Yangi ko&apos;chiriladigan guruh *
                </label>
                <select
                  required
                ...
                >
                  ...
                  {(groupsData || [])
                    .filter((g: any) => g.id !== transferData.from_group_id)
                    .map((g: any) => (
              ...
              <div className="pt-2 flex items-center justify-end gap-3">
                ...
                >
                  ...
                  <span>Ko&apos;chirishni tasdiqlash</span>
                </button>
              </div>
            </form>
          </div>
